...
function get_products() {
  ...
  connect_to_db();
  $sql = "SELECT p.product_id, p.title, v.title AS variant_title, v.sku, v.price, v.qty, s.store_name, c.channel, ven.vendor 
        FROM product AS p INNER JOIN variant AS v ON p.product_id=v.product_id
        INNER JOIN store AS s ON v.store_id = s.store_id
        INNER JOIN channel AS c ON c.channel_id=s.channel_id
        INNER JOIN vendor AS ven ON ven.vendor_id=s.vendor_id";
...
function sync_products() {
  ...
  connect_to_db();
  $sql = "SELECT c.channel_id, c.channel, c.products_api, s.store_id, s.store_name, s.api_key, s.api_password, s.access_token, v.vendor
FROM store AS s
INNER JOIN vendor AS v ON s.vendor_id = v.vendor_id
INNER JOIN channel AS c ON s.channel_id = c.channel_id
";
...
// save into DB: to do in just ONE query
function save_product($store_id, $product) {
  ...
  $sql = "SELECT product_id FROM variant WHERE sku IN ('". implode("','", $skus) ."') ORDER BY product_id LIMIT 1";
 
  //@todo: WHAT if there are more than 1 product IDs
